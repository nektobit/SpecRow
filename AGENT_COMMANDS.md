...
If proposal input contains `brief` or `бриф:`, the following text is the original human-side task description: context, expectations, observed problems, business intent, constraints, or user-facing goals. The agent uses it as source material and converts it into structured proposal content; the brief is not final implementation instructions by itself.
