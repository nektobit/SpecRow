...
2. Inspect status through the selected adapter: call `specrow_project_status` with the target `projectRoot`, or run the bundled CLI `status` from that root.
3. If the workspace is not initialized, call `specrow_init` with the same `projectRoot`, or run `init --language <code>` from that root.
4. Validate through the same adapter and report blocking issues before starting another workflow phase.
