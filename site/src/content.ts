...
export const docContent: Record<LocaleCode, Record<PageSlug, PageContent>> = {
  en: {
    ...
    'mcp-server': {
      ...
      blocks: [
        { type: 'section', heading: 'How It Works', paragraphs: ['The MCP server runs as an agent-managed stdio process and is bound to the project root. After the agent restarts, it receives the SpecRow tool namespace and can call operations directly without shell commands.', 'Users usually do not type the full technical name such as mcp__specrow__.specrow_validate. It is enough to write an intent like specrow validate, specrow review, or ask the agent to check SpecRow. The agent maps that intent to the right MCP tool.'], commands: ['specrow validate', 'specrow proposal Add passwordless sign-in', 'specrow build <change-name>'] },
  ...
  ru: {
    ...
    'mcp-server': {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Read-only tools', paragraphs: ['Эти вызовы безопасны для чтения: они проверяют структуру, показывают активные изменения, возвращают статус и готовят agent-readable context.'], commands: ['mcp__specrow__.specrow_validate({})', 'mcp__specrow__.specrow_validate({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_list({})', 'mcp__specrow__.specrow_status({})', 'mcp__specrow__.specrow_status({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_context({})', 'mcp__specrow__.specrow_context({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_workflow_guide({})'] },
  ...
  es: {
    ...
    'mcp-server': {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Read-only tools', paragraphs: ['Estas llamadas son lecturas seguras: validan la estructura, muestran cambios activos, devuelven status y preparan contexto legible para agentes.'], commands: ['mcp__specrow__.specrow_validate({})', 'mcp__specrow__.specrow_validate({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_list({})', 'mcp__specrow__.specrow_status({})', 'mcp__specrow__.specrow_status({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_context({})', 'mcp__specrow__.specrow_context({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_workflow_guide({})'] },
  ...
  'zh-CN': {
    ...
    'mcp-server': {
      ...
      blocks: [
        { type: 'section', heading: '工作方式', paragraphs: ['MCP server 作为代理管理的 stdio 进程运行，并绑定到项目根目录。代理重启后会获得 SpecRow tool namespace，并可以直接调用操作，而不需要 shell 命令。', '用户通常不需要输入 mcp__specrow__.specrow_validate 这样的完整技术名称。只要写出意图即可，例如 specrow validate、specrow review，或让代理检查 SpecRow。代理会把该意图映射到正确的 MCP tool。'], commands: ['specrow validate', 'specrow proposal 添加无密码登录', 'specrow build <change-name>'] },
        ...
        { type: 'section', heading: 'Read-only tools', paragraphs: ['这些调用是安全读取：验证结构、显示活跃变更、返回状态，并准备代理可读上下文。'], commands: ['mcp__specrow__.specrow_validate({})', 'mcp__specrow__.specrow_validate({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_list({})', 'mcp__specrow__.specrow_status({})', 'mcp__specrow__.specrow_status({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_context({})', 'mcp__specrow__.specrow_context({ "changeName": "<change-name>" })', 'mcp__specrow__.specrow_workflow_guide({})'] },
