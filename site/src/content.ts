...
export const pageContracts: Record<PageSlug, { revision: number; sourceLocale: LocaleCode; sectionIds: readonly string[] }> = {
  ...
  instructions: { revision: 5, sourceLocale: 'en', sectionIds: ['sdd-introduction', 'requirements', 'choose-client', 'install-copilot', 'install-vscode', 'install-kiro', 'marketplace-clients', 'unsupported-clients', 'package-contents', 'first-check', 'project-selection', 'first-workflow', 'acceptance-boundary', 'cli-automation'] },
  workflow: { revision: 3, sourceLocale: 'en', sectionIds: ['states', 'explore', 'proposal', 'review', 'build', 'revise', 'accept-and-archive'] },
  'agent-commands': { revision: 4, sourceLocale: 'en', sectionIds: ['intent-contract', 'explore', 'proposal', 'review', 'build', 'revise', 'accept-and-archive', 'stop-points'] },
...
export const reviewedContentRevisions: Record<LocaleCode, Record<PageSlug, number>> = {
  en: { manifesto: 2, instructions: 5, workflow: 3, 'agent-commands': 4, 'mcp-server': 3, 'cli-reference': 3, migration: 1, templates: 2, localization: 3, 'validation-lifecycle': 2, 'sd-development': 2 },
  ru: { manifesto: 2, instructions: 5, workflow: 3, 'agent-commands': 4, 'mcp-server': 3, 'cli-reference': 3, migration: 1, templates: 2, localization: 3, 'validation-lifecycle': 2, 'sd-development': 2 },
  es: { manifesto: 2, instructions: 5, workflow: 3, 'agent-commands': 4, 'mcp-server': 3, 'cli-reference': 3, migration: 1, templates: 2, localization: 3, 'validation-lifecycle': 2, 'sd-development': 2 },
  'zh-CN': { manifesto: 2, instructions: 5, workflow: 3, 'agent-commands': 4, 'mcp-server': 3, 'cli-reference': 3, migration: 1, templates: 2, localization: 3, 'validation-lifecycle': 2, 'sd-development': 2 },
}
  ...
  en: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { id: 'requirements', type: 'list-section', heading: 'Requirements', intro: 'Every supported installation needs:', items: ['Node.js 20 or newer available on PATH.', 'Filesystem access to the target project.', 'Agent Plugins clients need Agent Skills plus local stdio MCP support. Codex desktop and Codex CLI instead need a local shell to run the bundled CLI adapter.'], outro: 'Install the plugin through the client. Installing a separate global SpecRow CLI is not required and does not register the plugin.' },
        { id: 'choose-client', type: 'list-section', heading: 'Choose Your Agent', intro: 'Installation support is intentionally explicit:', items: ['Codex desktop/CLI: use the skills-only package with its bundled local CLI adapter. One-click installation becomes available after OpenAI directory approval.', 'GitHub Copilot CLI: direct one-command installation from GitHub.', 'GitHub Copilot in VS Code: install from the repository in the Plugins UI.', 'Kiro IDE: import the repository as a custom Power.', 'Cursor: the compatible Agent Plugins package is included; the simple public path depends on marketplace publication.', 'Hermes Agent, OpenClaw, Grok Bot, and NanoClaw: compatible with the Agent Plugins format, but not yet tested by SpecRow.'], outro: 'These paths are documented by their client vendors, but SpecRow does not yet run client-specific end-to-end installation tests.' },
        ...
        { id: 'marketplace-clients', type: 'list-section', heading: 'Codex And Cursor', intro: 'The repository contains separate adapters for the clients, but public one-click installation still requires listing:', items: ['Codex desktop and Codex CLI use a skills-only package with a bundled CLI. It does not need an MCP server or a public endpoint. The Codex IDE extension is not yet supported or tested by SpecRow.', 'Cursor uses the portable Agent Plugins package with the skill and local stdio MCP server. Install from Customize or its marketplace after SpecRow is listed. Availability can depend on the Cursor plan or administrator policy. An MCP-only deep link is not a complete SpecRow install because it omits the skill.'], outro: 'Until those listings exist, do not present a marketplace button as working.' },
        ...
        { id: 'package-contents', type: 'code-section', heading: 'What Gets Installed', intro: 'The repository ships both client adapters:', code: 'plugin.json\nmcp.json\n.codex-plugin/plugin.json\nskills/specrow/SKILL.md\nskills/specrow/scripts/specrow-cli.cjs\nruntime/specrow-mcp.cjs', outro: 'Agent Plugins clients use the root manifest and MCP runtime. Codex uses the skills-only manifest and bundled CLI.' },
        ...
        { id: 'project-selection', type: 'section', heading: 'Select The Correct Project', paragraphs: ['With MCP, the agent passes an absolute projectRoot when the client exposes no roots or several roots remain ambiguous. If exactly one initialized SpecRow workspace exists among several roots, the server selects it automatically. In Codex, bundled CLI commands run with the target project as their working directory. The plugin installation directory is never a user workspace.'] },
    ...
    'agent-commands': {
      ...
      description: 'Short SpecRow phrases are workflow requests. The agent maps them to the available client adapter—MCP or Codex bundled CLI; they are not literal CLI commands.',
      blocks: [
        { id: 'intent-contract', type: 'section', heading: 'Intent Contract', paragraphs: ['A phrase such as specrow build asks the agent to perform the corresponding workflow through the client adapter: MCP when available, or the bundled CLI in Codex. It is not a literal shell command entered by the user.'] },
  ...
  ru: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { id: 'requirements', type: 'list-section', heading: 'Требования', intro: 'Для любого поддерживаемого способа установки потребуются:', items: ['Node.js 20 или новее, доступный через PATH.', 'Доступ к файловой системе целевого проекта.', 'Клиентам Agent Plugins нужны Agent Skills и поддержка локальных stdio MCP-серверов. Codex desktop и Codex CLI вместо этого нужен локальный shell для запуска встроенного CLI-адаптера.'], outro: 'Устанавливайте плагин через клиент. Отдельная глобальная установка SpecRow CLI не требуется и не регистрирует плагин.' },
        { id: 'choose-client', type: 'list-section', heading: 'Выберите агента', intro: 'Поддержка установки разделена явно:', items: ['Codex desktop/CLI: используйте skills-only пакет со встроенным локальным CLI-адаптером. Установка одной кнопкой станет доступна после одобрения в каталоге OpenAI.', 'GitHub Copilot CLI: прямая установка из GitHub одной командой.', 'GitHub Copilot в VS Code: установка из репозитория через интерфейс Plugins.', 'Kiro IDE: импорт репозитория как custom Power.', 'Cursor: совместимый пакет Agent Plugins уже входит в репозиторий; простой публичный путь зависит от публикации в marketplace.', 'Hermes Agent, OpenClaw, Grok Bot и NanoClaw: совместимы с форматом Agent Plugins, но пока не проверены командой SpecRow.'], outro: 'Эти пути описаны разработчиками клиентов, но SpecRow пока не запускает клиентские end-to-end тесты установки.' },
        ...
        { id: 'marketplace-clients', type: 'list-section', heading: 'Codex и Cursor', intro: 'В репозитории есть отдельные адаптеры для клиентов, но публичная установка одной кнопкой требует публикации:', items: ['Codex desktop и Codex CLI используют skills-only пакет со встроенным CLI. Ему не нужны MCP-сервер или публичный endpoint. Codex IDE extension пока не поддержан и не проверен SpecRow.', 'Cursor использует переносимый пакет Agent Plugins со skill и локальным stdio MCP-сервером. Установка через Customize или marketplace станет доступна после публикации SpecRow. Доступность может зависеть от тарифа Cursor или политики администратора. MCP-only deeplink не является полной установкой — он не добавляет skill.'], outro: 'Пока публикации нет, нельзя показывать marketplace-кнопку как рабочую.' },
        ...
        { id: 'package-contents', type: 'code-section', heading: 'Что устанавливается', intro: 'Репозиторий поставляет оба клиентских адаптера:', code: 'plugin.json\nmcp.json\n.codex-plugin/plugin.json\nskills/specrow/SKILL.md\nskills/specrow/scripts/specrow-cli.cjs\nruntime/specrow-mcp.cjs', outro: 'Клиенты Agent Plugins используют корневой manifest и MCP runtime. Codex использует skills-only manifest и встроенный CLI.' },
        ...
        { id: 'project-selection', type: 'section', heading: 'Выбор правильного проекта', paragraphs: ['При работе через MCP агент передаёт абсолютный projectRoot, если клиент не передаёт roots или несколько roots остаются неоднозначными. Если среди нескольких roots есть ровно один инициализированный SpecRow workspace, сервер выберет его автоматически. В Codex команды встроенного CLI запускаются с целевым проектом в качестве рабочего каталога. Каталог установки плагина никогда не считается пользовательским workspace.'] },
    ...
    'agent-commands': {
      ...
      description: 'Короткие фразы SpecRow — запросы workflow. Агент сопоставляет их с доступным клиентским адаптером — MCP или встроенным CLI для Codex; это не буквальные CLI-команды.',
      blocks: [
        { id: 'intent-contract', type: 'section', heading: 'Контракт намерений', paragraphs: ['Фраза вроде specrow build просит агента выполнить соответствующий workflow через клиентский адаптер: MCP, если он доступен, или встроенный CLI в Codex. Это не буквальная shell-команда, которую вводит пользователь.'] },
  ...
  es: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { id: 'requirements', type: 'list-section', heading: 'Requisitos', intro: 'Cada instalación compatible necesita:', items: ['Node.js 20 o posterior disponible en PATH.', 'Acceso al sistema de archivos del proyecto de destino.', 'Los clientes de Agent Plugins necesitan Agent Skills y compatibilidad con servidores MCP stdio locales. Codex desktop y Codex CLI necesitan, en cambio, un shell local para ejecutar el adaptador CLI incluido.'], outro: 'Instala el plugin desde el cliente. No se requiere una instalación global separada de la CLI de SpecRow y esta no registra el plugin.' },
        { id: 'choose-client', type: 'list-section', heading: 'Elige tu agente', intro: 'El soporte de instalación se indica de forma explícita:', items: ['Codex desktop/CLI: usa el paquete solo de skills con su adaptador CLI local incluido. La instalación con un clic estará disponible después de la aprobación en el directorio de OpenAI.', 'GitHub Copilot CLI: instalación directa desde GitHub con un comando.', 'GitHub Copilot en VS Code: instalación desde el repositorio en la interfaz Plugins.', 'Kiro IDE: importa el repositorio como Power personalizado.', 'Cursor: el paquete compatible con Agent Plugins ya está incluido; la ruta pública sencilla depende de su publicación en el marketplace.', 'Hermes Agent, OpenClaw, Grok Bot y NanoClaw: compatibles con el formato Agent Plugins, pero aún no probados por SpecRow.'], outro: 'Estos caminos están documentados por los proveedores de los clientes, pero SpecRow todavía no ejecuta pruebas end-to-end de instalación específicas de cada cliente.' },
        ...
        { id: 'marketplace-clients', type: 'list-section', heading: 'Codex y Cursor', intro: 'El repositorio contiene adaptadores separados para los clientes, pero la instalación pública con un clic aún requiere publicación:', items: ['Codex desktop y Codex CLI usan un paquete solo de skills con una CLI incluida. No necesita servidor MCP ni endpoint público. La extensión Codex para IDE aún no está soportada ni probada por SpecRow.', 'Cursor usa el paquete portátil de Agent Plugins con la skill y el servidor MCP stdio local. Instala desde Customize o su marketplace cuando SpecRow esté publicado. La disponibilidad puede depender del plan de Cursor o de la política del administrador. Un deeplink solo de MCP no es una instalación completa porque omite la skill.'], outro: 'Hasta entonces, no presentes un botón de marketplace como si funcionara.' },
        ...
        { id: 'package-contents', type: 'code-section', heading: 'Qué se instala', intro: 'El repositorio distribuye ambos adaptadores de cliente:', code: 'plugin.json\nmcp.json\n.codex-plugin/plugin.json\nskills/specrow/SKILL.md\nskills/specrow/scripts/specrow-cli.cjs\nruntime/specrow-mcp.cjs', outro: 'Los clientes de Agent Plugins usan el manifest raíz y el runtime MCP. Codex usa el manifest solo de skills y la CLI incluida.' },
        ...
        { id: 'project-selection', type: 'section', heading: 'Seleccionar el proyecto correcto', paragraphs: ['Con MCP, el agente pasa el projectRoot absoluto si el cliente no expone raíces o varias raíces siguen siendo ambiguas. Si entre varias raíces existe un solo workspace SpecRow inicializado, el servidor puede elegirlo automáticamente. En Codex, los comandos de la CLI incluida se ejecutan con el proyecto de destino como directorio de trabajo. El directorio del plugin nunca es un workspace del usuario.'] },
    ...
    'agent-commands': {
      ...
      description: 'Las frases cortas de SpecRow son solicitudes de workflow. El agente las traduce al adaptador disponible —MCP o la CLI incluida para Codex—; no son comandos CLI literales.',
      blocks: [
        { id: 'intent-contract', type: 'section', heading: 'Contrato de intención', paragraphs: ['Una frase como specrow build pide al agente ejecutar el workflow mediante el adaptador del cliente: MCP cuando esté disponible o la CLI incluida en Codex. No es un comando shell literal introducido por el usuario.'] },
  ...
  'zh-CN': {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { id: 'requirements', type: 'list-section', heading: '要求', intro: '每种受支持的安装方式都需要：', items: ['PATH 中可用的 Node.js 20 或更高版本。', '能够访问目标项目的文件系统。', 'Agent Plugins 客户端需要 Agent Skills 和本地 stdio MCP 支持；Codex desktop 和 Codex CLI 则需要本地 shell 来运行随包提供的 CLI 适配器。'], outro: '请通过客户端安装插件。无需单独全局安装 SpecRow CLI，而且那样也不会在代理中注册插件。' },
        { id: 'choose-client', type: 'list-section', heading: '选择代理', intro: '安装支持范围明确如下：', items: ['Codex desktop/CLI：使用随附本地 CLI 适配器的纯 skills 包。通过 OpenAI 目录审核后即可一键安装。', 'GitHub Copilot CLI：一条命令直接从 GitHub 安装。', 'VS Code 中的 GitHub Copilot：通过 Plugins 界面从仓库安装。', 'Kiro IDE：将仓库导入为 custom Power。', 'Cursor：仓库已包含兼容的 Agent Plugins 包；简单的公开安装方式取决于 marketplace 发布。', 'Hermes Agent、OpenClaw、Grok Bot 和 NanoClaw：格式兼容 Agent Plugins，但尚未经过 SpecRow 测试。'], outro: '这些路径由客户端厂商提供文档，但 SpecRow 尚未运行特定客户端的端到端安装测试。' },
        ...
        { id: 'marketplace-clients', type: 'list-section', heading: 'Codex 和 Cursor', intro: '仓库为不同客户端提供了独立适配器，但公开的一键安装仍需先发布：', items: ['Codex desktop 和 Codex CLI 使用随包提供 CLI 的纯 skills 包。它不需要 MCP 服务器或公共 endpoint。Codex IDE 扩展尚未得到 SpecRow 支持或测试。', 'Cursor 使用包含 skill 和本地 stdio MCP 服务器的可移植 Agent Plugins 包。SpecRow 上架后可从 Customize 或 marketplace 安装。可用性可能取决于 Cursor 套餐或管理员策略。仅安装 MCP 的 deeplink 会遗漏 skill，因此不是完整安装。'], outro: '上架之前，不应把 marketplace 按钮展示为可用。' },
        ...
        { id: 'package-contents', type: 'code-section', heading: '安装内容', intro: '仓库同时分发两种客户端适配器：', code: 'plugin.json\nmcp.json\n.codex-plugin/plugin.json\nskills/specrow/SKILL.md\nskills/specrow/scripts/specrow-cli.cjs\nruntime/specrow-mcp.cjs', outro: 'Agent Plugins 客户端使用根 manifest 和 MCP runtime；Codex 使用纯 skills manifest 和随包提供的 CLI。' },
        ...
        { id: 'project-selection', type: 'section', heading: '选择正确的项目', paragraphs: ['使用 MCP 时，如果客户端未提供 roots，或多个 roots 仍然无法消除歧义，代理必须传入绝对 projectRoot。如果多个 roots 中只有一个已初始化的 SpecRow 工作区，服务器可以自动选择它。在 Codex 中，随包提供的 CLI 命令以目标项目作为工作目录运行。插件安装目录绝不是用户工作区。'] },
    ...
    'agent-commands': {
      eyebrow: '与代理协作',
      title: '意图，而非 CLI',
      description: '简短的 SpecRow 短语是工作流请求。代理会将其映射到可用的客户端适配器——MCP 或 Codex 随包提供的 CLI；它们不是字面 CLI 命令。',
      blocks: [
        { id: 'intent-contract', type: 'section', heading: '意图约定', paragraphs: ['像 specrow build 这样的短语要求代理通过客户端适配器执行对应工作流：可用时使用 MCP，在 Codex 中则使用随包提供的 CLI。它不是用户输入的字面 shell 命令。'] },
...
export const sourceContentDigests: Record<PageSlug, string> = {
  manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0',
}

export const reviewedContentDigests: Record<LocaleCode, Record<PageSlug, string>> = {
  en: { manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0' },
  ru: { manifesto: 'a2f81a18', instructions: '03dbac30', workflow: '99fa35e1', 'agent-commands': '665284c9', 'mcp-server': '771d52f6', 'cli-reference': 'df9e9e0e', migration: 'fc64ac2c', templates: '932fbef2', localization: '72bcdd02', 'validation-lifecycle': '98028ab5', 'sd-development': '8c43a099' },
  es: { manifesto: '4ab5377a', instructions: '7c834eb3', workflow: '948685ec', 'agent-commands': '864a8515', 'mcp-server': 'd70d9b93', 'cli-reference': '764b41a2', migration: 'be78a0cf', templates: '0ede6928', localization: 'f8b58b84', 'validation-lifecycle': 'd1ff75a2', 'sd-development': 'bf4f5f0b' },
  'zh-CN': { manifesto: 'a97db9a6', instructions: 'cfa643c4', workflow: 'a30c3c0a', 'agent-commands': '6f7a49fe', 'mcp-server': '3f087378', 'cli-reference': '5297f92d', migration: '16f66eae', templates: '56f5c4b8', localization: 'dc01c907', 'validation-lifecycle': '890beab7', 'sd-development': '62636763' },
}

export const reviewedSourceDigests: Record<LocaleCode, Record<PageSlug, string>> = {
  en: { manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0' },
  ru: { manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0' },
  es: { manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0' },
  'zh-CN': { manifesto: 'b38eef9f', instructions: 'a5ad9766', workflow: '3f59002d', 'agent-commands': 'aaf042cf', 'mcp-server': 'a9cc9ee3', 'cli-reference': '46758e8f', migration: '2eeb3564', templates: 'f728d486', localization: 'e048c514', 'validation-lifecycle': '7b34c10b', 'sd-development': '1ff632c0' },
}
