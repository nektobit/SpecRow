  ...
  en: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Normal Use', paragraphs: ['After setup, tell the agent what you want with SpecRow agent commands. The agent may call the CLI, read .specrow files, run validation, and prepare context, but you do not need to memorize the CLI.'] },
        { type: 'command-section', heading: 'Manual CLI Fallback', intro: 'Use the CLI directly when you need automation, CI, or manual setup.', commands: ['npm i -g specrow', 'specrow init --language en --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'], outro: 'Without --tools, specrow init only creates the .specrow workspace.' },
  ...
  ru: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Обычная работа', paragraphs: ['После установки формулируйте задачу агенту через команды SpecRow. Агент может вызывать CLI, читать файлы .specrow, запускать валидацию и готовить контекст, но пользователю не нужно запоминать CLI.'] },
        { type: 'command-section', heading: 'Ручной CLI fallback', intro: 'Используйте CLI напрямую для автоматизации, CI или ручной установки.', commands: ['npm i -g specrow', 'specrow init --language ru --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'], outro: 'Без --tools команда specrow init создает только workspace .specrow.' },
  ...
  es: {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Uso normal', paragraphs: ['Después de la instalación, dile al agente lo que quieres con comandos de SpecRow. El agente puede llamar a la CLI, leer archivos .specrow, ejecutar validación y preparar contexto, pero no necesitas memorizar la CLI.'] },
        { type: 'command-section', heading: 'Fallback manual con CLI', intro: 'Usa la CLI directamente para automatización, CI o instalación manual.', commands: ['npm i -g specrow', 'specrow init --language es --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'], outro: 'Sin --tools, specrow init solo crea el workspace .specrow.' },
    ...
    'cli-reference': {
      ...
      blocks: [
        ...
        { type: 'section', heading: 'Integraciones de agentes', paragraphs: ['Instala o actualiza command, prompt, skill, rule, workflow y fallback instruction files para agentes compatibles. init solo instala integraciones cuando se pasa --tools o --detect.'], commands: ['specrow init --tools <tools>', 'specrow integrate --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'] },
  ...
  'zh-CN': {
    ...
    instructions: {
      ...
      blocks: [
        ...
        { type: 'section', heading: '常规使用', paragraphs: ['安装后，用 SpecRow 代理命令告诉代理你的意图。代理可以调用 CLI、读取 .specrow 文件、运行验证并准备上下文，但用户不需要记住 CLI。'] },
        { type: 'command-section', heading: '手动 CLI fallback', intro: '需要自动化、CI 或手动安装时，直接使用 CLI。', commands: ['npm i -g specrow', 'specrow init --language zh-CN --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'], outro: '没有 --tools 时，specrow init 只创建 .specrow workspace。' },
    ...
    'cli-reference': {
      ...
      blocks: [
        ...
        { type: 'section', heading: '代理集成', paragraphs: ['为支持的代理安装或更新 command、prompt、skill、rule、workflow 和 fallback instruction files。只有传入 --tools 或 --detect 时，init 才会安装集成。'], commands: ['specrow init --tools <tools>', 'specrow integrate --tools codex,claude,cursor,windsurf,generic', 'specrow integrate --detect', 'specrow update', 'specrow integrations status'] },
    ...
    localization: {
      ...
      blocks: [
        ...
        { type: 'section', heading: '添加语言', paragraphs: ['将语言加入支持的 locale 列表，为模板、系统消息、代理命令和集成文本创建完整 runtime 资源，然后添加对应文档。只有 locale 验证在没有缺失资源和 placeholder 差异的情况下通过，新语言才算完成。'], commands: ['npm run locale:validate'] },
